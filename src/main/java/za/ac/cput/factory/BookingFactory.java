...
public class BookingFactory {
    public static Booking buildBooking(String bookingId, Car car, String startDate, String returnDate, String pickUpLocation, String dropOffLocation, String status, double totalPrice){
        if(Helper.isNullOrEmpty(bookingId) || Helper.isNullOrEmpty(startDate) || Helper.isNullOrEmpty(returnDate) || Helper.isNullOrEmpty(status) || car == null
                || Helper.isNullOrEmpty(pickUpLocation) || Helper.isNullOrEmpty(dropOffLocation)|| totalPrice < 0 )
            return null;

        return new Booking.Builder().setBookingId(bookingId).setCar(car).setStartDate(startDate).setReturnDate(returnDate).setPickUpLocation(pickUpLocation).setDropOffLocation(dropOffLocation)
